# 设计决策记录

## 决策 34: 卧龙股池以最新日度物化批次为唯一成分来源，实时行情仅作覆盖层

**日期**: 2026-08-11

**背景**: 单因子指数已完成月度调仓和日度 NAV 物化，但用户无法查看各因子当前高分成分。若直接读取“最新调仓”或把实时价格写回成分表，会让当前榜单与最新指数日度、数据日期或历史复盘口径发生漂移。

**决策**:
1. 新增 `/stock-pool` 和 `GET /api/live/factor-pool`，固定展示价值、股息率、成长、质量、动量、小市值、低波动 7 榜，每榜取 `rank ASC` 的 Top10。
2. 每个榜单的当前成分必须从该指数最新 `factor_index_daily` 的 `rebalance_id` 回溯到 `factor_index_constituents`，而不是单独读取最新 `factor_index_rebalances`。
3. 成分、排名、因子分、行业、信号日价格是月度冻结事实；实时价格、当日涨跌、成交额是服务端批量获取的短缓存覆盖层，禁止回写因子事实表或影响排序。
4. 行情拉取失败时接口仍返回完整榜单，`quote_status=unavailable` 且行级行情为 null；不得删除成分、不得用 0 伪造行情。
5. 用户日期以 `source_trade_date` 表达因子事实数据日期；行情使用独立 `quote_updated_at`。页面提示因子分只在同榜内可比。

**原因**:
1. 将 current constituent 与指数最新日度物化行绑定，可保证 NAV、状态、数据日期、成分批次一致。
2. 服务端去重后一次批量拉取最多 70 个标的，可复用 live 的 10 秒缓存并避免前端逐行请求。
3. 事实层与行情层分离后，盘中与行情失败都不会污染月度调仓的可复盘性。

**替代方案（未选）**:
- 仅从最新调仓表读取 Top10：否决，调仓与日度物化可能不同步，容易重现“当前名单与最新结果快照不一致”。
- 将实时行情更新到 `factor_index_constituents`：否决，会把短生命周期数据混入月度事实并破坏复盘。
- 前端逐只请求行情：否决，70 行会放大请求、绕过服务端缓存且增加失败噪音。

**风险与代价**:
- 7 榜之间同一股票可重复出现，这是多因子暴露的正常表达，不做跨榜去重。
- 当前实时行情源短暂不可用时价格列会为空；这是刻意选择“保留事实、不伪造行情”的降级。
- 首期仅展示基础行情，估值、资金和因子专属原始指标后续按数据质量再扩展。

## 决策 33: 单因子指数 NAV 自愈机制（覆盖率门槛 + partial 级联重算 + 停牌/缺数分级）

**日期**: 2026-07-25

**背景**: live-trading 因子指数卡长期显示「部分数据」与「X/50 只成分股缺少完整日线」。调查（2026-07-24）确认三层根因：① NAV 行「先到先得、写过不重算」——`ListTradeDates` 只看全表 distinct 日期，`computeDailyForTradeDate` 对已有行永久跳过，日线晚到时落库的残缺行永不自愈，且 NAV 链式推进会把错误向后传播；② `is_delisted_name` 只匹配「退市」前缀漏「X退」后缀，92 只退市整理股 `is_active=1` 进入小市值指数，退市后日线断更；③ docker-compose 把 `FACTOR_LAB_DAILY_BARS_SOURCE` 钉死 `tencent`，腾讯 fqkline 大面积 501 时无降级，全市场日线残缺并连带 phase1 选股快照失败。

**决策**:
1. **覆盖率门槛**（P1）：`SyncDaily` 先取每交易日日线覆盖数（`ListTradeDateBarCounts`），以全表最大单日覆盖数为基线，当日覆盖 < 基线 × `MinBarCoverageRatio`（默认 0.9，env `FACTOR_INDEX_MIN_BAR_COVERAGE_RATIO`）时跳过该日，等数据补齐后再算。`<=0` 表示不启用。
2. **partial 级联重算**（P1）：`computeDailyForTradeDate` 只在「已有行且 status=completed」时跳过；已有 partial/pending 行时先级联删除该指数当日及之后全部 daily 行，由外层按日期升序的循环顺序重算，NAV 链自动从最近一个 completed 行续接。admin recompute(reset) 的手工修复路径保持不变。
3. **停牌/退市与数据缺失分级**（P2）：成分股当日无日线时，查 `factor_market_metrics` 当日或之前最近一条记录，`is_suspended=1` 则计入 `SuspendedPriceCount`（按 0 收益在经济上正确，不触发 partial，警告文案为信息性「X/50 只成分股停牌或已退市，按 0 收益处理」）；否则计入缺失并触发 partial。取「最近一条」而非「当日」是为了覆盖退市股（数据源在其退市后停止推送快照）。`factor_index_daily` 与 overview 响应新增 `suspended_price_count` 字段；前端无需改动（任何 status 都渲染 warningText）。
4. **退市名口径统一**（P0）：`is_delisted_name` 改为 `"退" in name`，与四象限 `universe_filter.is_st_name` 完全一致；securities 每日全量 `INSERT OR REPLACE` 会自动重判存量股票，另用一次性 SQL 立即修复存量 92 只。
5. **日线源恢复降级链**（P0）：docker-compose（prod/local 两份）`FACTOR_LAB_DAILY_BARS_SOURCE` 默认值 `tencent` → `auto`（= baostock→tencent→eastmoney→akshare）。quant 增量脚本自身的 argparse 默认 `tencent` 保留（仅影响手工运行，是有意的快速通道）。
6. **Admin 可观测性**：`/api/admin/factor-index/status` 响应新增 `latest_trade_date_bar_count` / `bar_coverage_baseline` / `min_bar_coverage_ratio`，用于判断最新交易日是否被覆盖率门槛跳过。

**替代方案（未选）**:
- 「不用日线的因子指数忽略日线」：否决。警告产生于 NAV 计算（指数日收益=成分股当日涨跌幅等权平均），7 个指数无一例外需要每只成分股每日收盘价，与因子评分是否用日线无关；评分层面价值/股息率/小市值/质量确实不用日线，但成长含 20% performance_1y，动量/低波动重度依赖日线。
- 重算时只覆写单行不级联：否决。NAV 按日链式推进，单行修正后其后所有行的 prevNAV 都已污染，必须级联。
- 停牌判定只看当日 metrics 行：否决。退市股在退市后无当日快照，会被误判为数据缺失继续触发 partial。

**风险与代价**:
- 级联重算在启动 catch-up 时可能一次重算多个交易日（当前历史 ~300 交易日 × 7 指数，实测秒级完成），成本可接受。
- 覆盖率门槛在数据源长期残缺时会让最新交易日 NAV 持续跳过（页面数据日期停留），这是刻意的「宁可晚更新不可错更新」；admin status 的覆盖率字段用于暴露该状态。
- 退市股在月内仍留在成分股中（月内不调仓是既定口径），按停牌/退市 0 收益处理并提示，下月调仓自动剔除。

**关联**: BP-025 / BP-026 / BP-027 / BP-028

## 决策 28: V2 generateFacts 重建换仓 diff 逻辑（BUY/SELL/HOLD）

**日期**: 2026-07-16

**背景**: V2 `generateFacts` 缺失与前一交易日持仓的 diff 逻辑，导致所有成交记录无条件记为 BUY。V1 的 `computeAndPersistSimPortfolioDay` 有完整 diff（previousMap vs selectedMap → SELL/BUY/HOLD），V2 重写时丢失。

**决策**:
在 `generateFacts` 中重建 diff 逻辑，对齐 V1 行为：
- 构建 `previousMap`（前日 positions）与 `selectedMap`（当日 selection items），以 `snapshotPriceHintKey(code, exchange)` 为 key
- 并集遍历，按存在性分类：仅 previous → SELL / 仅 selected → BUY / 两边都有 → HOLD
- reason 复用 V1 常量：`drop_out_top4` / `enter_top4` / `stay_top4`
- SELL 的 `tradePrice` 通过 `openPriceResolver` 回退获取（被卖出的股票不在当日 priceRequirements 中）
- `OldWeight/OldShares` 从前日 position 填充，`NewWeight/NewShares` 从当日选股填充，`ShareDelta = NewShares - OldShares`

**替代方案（未选）**:
1. 直接复用 V1 的 `computeAndPersistSimPortfolioDay` — 拒绝，V2 pipeline 结构（stages/priceRequirements/watermark）与 V1 不同，强耦合会引入更多不一致
2. 在 repo 层做 diff — 拒绝，diff 是业务逻辑应在 service 层

**风险与代价**:
- 历史存量全 BUY 数据需重算纠正（可通过 pipeline `Run` 按日 `ReplaceSimPortfolioV2FactDate` 覆盖）
- SELL 交易依赖 `openPriceResolver`，若未注入则 `tradePrice=0`（不影响 action/reason 正确性，仅影响展示）

**关联**: BP-006

## 决策 27: 重建市场时保留已修复价格需求 + dailyBarFetcher 兜底解析

**日期**: 2026-07-05

**背景**: Admin 模拟组合板块中，用户通过「重拉该日缺失价格」修复的价格在点击「确认应用并重建该市场」后会回退为缺失。根因有二：(1) `ReplaceSimPortfolioV2PriceRequirements` 破坏性 DELETE+INSERT 丢弃已 satisfied 行；(2) `resolveSinglePriceRequirement` 解析链不含 `dailyBarFetcher`，无法从腾讯 API 兜底。

**决策**:
1. `ReplaceSimPortfolioV2PriceRequirements`（repository.go）改为保留 satisfied 行：先查出已有 satisfied 行 → 删除全部 → 合并插入（satisfied 行保留原 Price/Source/Status，新行插入 pending）。
2. `resolveSinglePriceRequirement`（service.go）在 priceResolver 之后增加 `dailyBarFetcher` 兜底，source 标记为 `daily_bar_fallback`。

**原因**:
1. 已修复的价格需求（无论来自 BackfillDailyBars 还是 RetryResolvePrices）代表运维已确认的数据，重建时不应丢弃。
2. 解析链与修复链的数据源不一致是根因之一；加入 dailyBarFetcher 兜底确保两条路径都能从腾讯 API 获取价格。

**替代方案**:
- BackfillDailyBars 成功时同时写入 `price_overrides` 表（持久化）：否决。这会改变 overrides 表的语义（本应只存人工覆盖），且 backfill 价格可能随 API 数据修正而变化，不适合持久化。
- 重建时不调用 ReplaceSimPortfolioV2PriceRequirements，只对新增股票创建行：否决。selection 变化时需要清理旧股票的行，Replace 的清理职责仍需保留。

**风险与代价**:
- Replace 逻辑增加一次 SELECT 查询（仅查 satisfied 行），当前每组合每天最多 4×2=8 行，成本可忽略。
- dailyBarFetcher 兜底会增加重建时的 API 调用，但仅在 priceLookupResolver/priceResolver 都返回 0 时触发，正常情况下不增加调用。
- satisfied 行保留意味着 selection 变化（股票换出）时，旧股票的 satisfied 行不会自动清理——但这些行的 TradeDate/Code 不在新的 selection 范围内，不会影响 facts 生成（generateFacts 只读 selection items 对应的 reqs）。

**测试**: `TestSimPortfolioV2RebuildPreservesBackfilledPrices`（方案一回归）、`TestSimPortfolioV2ResolveUsesDailyBarFallback`（方案二回归）。

## 决策 26: 模拟组合事实表 Admin 必须同时暴露流程阶段与同步结果

**日期**: 2026-07-01

**背景**: `/portfolio-tracking` 已切到新事实表链路，但本地排查发现部分组合只有 seeded baseline，`portfolio_position` / `portfolio_trade` / `portfolio_metrics` 为空。前置开盘价和 legacy 结果表都可能已有数据，但页面仍为空。原 admin 面板只展示缺开盘价和状态，且「补齐建仓开盘价」容易被误解为会生成持仓。

**决策**:
1. Admin 状态表必须区分「前置价格是否就绪」与「事实表是否已生成」，新增事实表行数、`baseline_only`、`can_sync`、下一段 `signal_date -> trade_date` 和 `action_hint`。
2. Admin 文案明确固定运维顺序：先补开盘价、再同步事实表、最后验证一致性；补开盘价不生成持仓/调仓。
3. `SyncSimPortfolios` 不再只返回 error，而是返回 per-portfolio 同步摘要，包括锚点、最新信号、生成估值日数量、最后生成日期和阻断原因。
4. 四象限 bulk-save 成功后的自动同步继续异步执行，但日志必须输出本轮生成估值日数量，避免 silent no-op。

**原因**:
1. seeded baseline 是合法中间态，但对运维来说必须可见，否则会误判为页面或前端问题。
2. 开盘价补齐只是前置动作，事实表生成是独立动作；把两者混在一个状态里会导致误操作。
3. per-portfolio 摘要能快速定位是哪个组合未推进、为什么未推进，避免只看到全局成功。

**替代方案**:
- 让「补齐建仓开盘价」自动同步事实表：否决。补价和写事实表是不同副作用，合并会降低可排障性。
- 只在前端加说明文案，不改后端响应：否决。缺少 per-portfolio 结果时，自动同步和手动同步仍不可观测。
- 新增独立同步日志表：暂缓。当前低频运维先用同步响应和 bulk-save 日志满足可观测性；若后续需要历史审计再持久化。

**风险与代价**:
- Admin status 查询增加少量 count 查询；当前组合固定为 4 个，成本可接受。
- 同步响应结构变更为对象摘要；现有 admin handler 测试已覆盖 `items`，前端按新结构展示。

## 决策 25: MiniChart Y 轴采用自适应 min/max + padding 而非起点归零

**日期**: 2026-07-01

**背景**: `/live-trading` 市场行情页的单因子指数卡与大盘指数卡共用 `MiniChart`（Canvas 迷你图）。原实现 Y 轴固定从 0 缩放到 `max(values)`，导致因子指数 NAV（0.98~1.08）真实波动只占画布约 8%、上证综指（3100~3250）约 4%，所有曲线「看起来都很平」，无法反映真实波动。

**决策**:
1. Y 轴从固定 [0, max] 改为数据 [dataMin − pad, dataMax + pad] 自适应，`pad = max(range*0.15, absMax*0.01)`。
2. 持平 / 极小波动（range/absMax < 0.1%）时以数据中点为中心构建 ±0.5% 虚拟范围；全 0 退化为 [0,1]。
3. 刻度值显示真实数值（不归零、不缩放），精度按量级自适应（≥100 取整，否则 2 位）。
4. 缩放逻辑抽成纯函数 `lib/mini-chart-scale.js`（`buildYAxisScale` 等），便于单测。
5. `MiniChart` 新增可选 props `yAxisMode`（默认 `auto`）/ `valuePrecision` / `baselineValue`，现有调用零改动即生效。
6. 保留 `zero-based` 模式给未来柱状图等必须从 0 起算的场景。

**原因**:
1. 卡片头部已展示绝对 NAV/点位与涨跌幅百分比，曲线只承担「辅助呈现形状与拐点」职责，不必再承担跨指数对比。
2. 自适应 min/max 是 Bloomberg、东方财富、同花顺等金融图表的标准做法，保留绝对值语义，符合用户直觉。
3. 改动集中在 MiniChart 内部 + 一个纯函数文件，不触及数据层与卡片布局，风险最小。

**替代方案**:
- 起点归零（所有点减起点值，Y 轴表相对变化）：否决。改变曲线语义，与卡片上方绝对值/涨跌幅文本割裂；起点本身随机，归零后曲线形状对起点敏感。
- 百分比归一化（起点=100%）：否决。完全丢失绝对值，与已有涨跌幅文本重复。
- 引入 Recharts/ECharts：否决。当前 Canvas 实现仅 130+ 行，引入第三方库带来 50KB+ 包体积与主题适配成本，收益不匹配。

**风险与代价**:
- 自适应会视觉放大波动，用户可能误以为「涨很多」。缓解：Y 轴刻度显示真实数值，卡片头部涨跌幅文本为权威信息。
- 跨卡片对比失真（每张卡 Y 轴范围不同）。缓解：sparkline 本就不适合精确跨卡对比，卡片头部涨跌幅是跨卡对比权威依据。
- `MiniChart` 硬编码 `rgba(255,255,255,...)` 文字/网格色未顺带修复，浅色主题下 Y 轴可读性差，作为独立后续项。

**关联文档**: `.ai/specs/mini-chart-yaxis/`

## 决策 24: 模拟组合事实表链路新增补开盘价补偿与缺口可观测性

**日期**: 2026-07-01

**背景**: `/portfolio-tracking` 四个模拟组合因建仓开盘价填充链路断裂（09:25 worker 错过窗口或 market price 行未先生成），导致事实表只停留在 seeded baseline，页面永久显示「等待下一交易日开盘价」且无任何失败状态暴露。

**决策**:
1. 新增 `BackfillSimPortfolioOpenPrices` service 方法，从日线 open 精确补齐 T+1 建仓开盘价，支持 `latest_only` 轻量模式和按组合/市场筛选。
2. 新增 `POST /api/admin/portfolio-tracking/backfill-open-prices` API 与 admin 按钮「补齐建仓开盘价」，作为新口径事实表链路的独立运维入口。
3. `SyncSimPortfolios` 内部先执行一次 `backfillSimPortfolioOpenPrices(latest_only=true)` 再同步事实表，具备最小自愈能力。
4. `RecomputeSimPortfolios` 内部先执行全量 `backfillSimPortfolioOpenPrices` 再重算，避免重算时因缺价失败。
5. admin status 响应新增 `missing_open_price_count` 和 `missing_close_price_count` 字段；admin 状态表新增「缺开盘价」列，琥珀色标记缺口。
6. `VerifySimPortfolios` 新增 `missing_open_price` 校验状态，检测持仓 `buy_price <= 0` 的记录。
7. 所有运维动作互斥，同一时间只能执行一个写库操作（sync / recompute / backfill / verify）。

**原因**:
1. 补开盘价是恢复事实表链路的最小修复动作，不应每次都从头重算。
2. sync/recompute 内部自动补价能覆盖大部分自动恢复场景，但 admin 仍需独立按钮用于排障。
3. 缺口计数让运维一眼看到"缺什么"而不是只看到"pending"。
4. verify 覆盖开盘价完整性可以防止事实表写入了无效 buy_price=0 的持仓。

**替代方案**:
- 只在 sync 内部补价，不加独立按钮：否决，运维需要分步排障能力。
- 新建 `portfolio_entry_price` 独立表去 legacy 依赖：暂缓，当前复用 legacy 表成本更低，封装在 service 后面即可。
- 用异步任务替代同步执行：暂缓，当前数据量小，同步返回足够快；后续数据量变大再改异步。

**风险与代价**:
- 补开盘价依赖外部日线接口，如果接口不可用仍会 pending；admin 应提示"可能是停牌、日线源未更新或目标交易日尚未到达"。
- latest_only 模式只补最新批次，历史缺口需要手动指定范围或走 recompute。
- 所有运维动作串行化，如果某个动作卡住，其他动作会等待；但这对当前低频运维场景可接受。

## 决策 23: 单因子指数以内嵌卡片形式接入 `/live-trading`，并采用后端全量物化链路

**日期**: 2026-06-30

**背景**: 产品希望在「市场行情」页直接观察 7 个单因子指数，不希望再引入额外详情页或前端临时计算。现有仓库已经具备 `factor_scores`、`factor_daily_bars` 和市场行情 dashboard，但尚未形成可直接展示的因子指数物化结果。

**决策**:
1. 单因子指数首期不新增详情页，7 张因子指数卡直接放在 `frontend/pages/live-trading.js` 的「核心指数卡片」上方。
2. 指数口径固定为：每月首个 A 股交易日收盘后按对应因子 score 选 Top50，下一交易日生效，等权 2%，月内不调仓。
3. 所有结果必须后端物化：新增独立 `factorindex` 模块和事实表，前端只请求 `/api/live/factor-index/overview` 展示 NAV、区间收益、状态和轻量净值曲线，不在页面内做收益/NAV 计算。
4. 调度拆为两段：月度调仓默认北京时间 20:10，日级 NAV 默认北京时间 20:30；worker 启动后先做一次 catch-up，同步历史调仓和日净值。
5. 管理后台统一挂到 `/admin/data`：提供 `/api/admin/factor-index/status` 查询 worker 与物化状态，`/api/admin/factor-index/recompute` 触发异步手动补算 / 从头重建，避免在 HTTP 请求内同步执行长任务。
6. 首期只支持 A 股，默认 7 个固定因子：价值、股息率、成长、质量、动量、小市值、低波动。

**原因**:
1. 因子指数的信息密度不足以支撑独立详情页，直接放在 `/live-trading` 更符合“同屏看市场与风格”的产品目标。
2. 后端物化可以锁定历史成分股和 NAV 曲线，避免前端按当下数据临时拼装导致口径漂移。
3. 调仓与日估值拆成两个时间点，可避免“当日调仓影响当日收益”的未来函数问题，并为上游收盘数据留出缓冲。
4. 单独 `factorindex` 模块能复用 Factor Lab 数据，又不污染现有 `portfolio_daily` / `portfolio_tracking` 组合语义。

**替代方案**:
- 新增 `/live-trading/factor-index` 详情页：否决，首期价值主要是快速横向对比，跳转会拉长路径。
- 前端直接根据成分股行情实时计算 NAV：否决，无法稳定复现历史口径，也违背“用户态读接口不隐式写库/算库”的长期原则。
- 复用 `portfolio_daily` 等模拟组合事实表：否决，指数方法论与模拟组合语义不同，混用会增加后续维护成本。

**风险与代价**:
- 因子与日线数据若在 20:10/20:30 尚未就绪，worker 需要依赖重试和启动 catch-up，运维上要接受“稍晚完成但仍保持 `source_trade_date` 正确”的状态。
- 首期卡片不展示 Top50 明细或 benchmark，对专业用户解释力偏弱；后续若信息量上升，再考虑抽屉或二级详情页。
- `factor_index_rebalances` 的生效日和结束日不能用自然日推算，后续所有扩展仍必须沿用交易日链路。

## 决策 22: 四象限自动计算固定为北京时间 20:00，并按市场交易日分别跳过

**日期**: 2026-06-29

**背景**: 原 `quadrant.Worker` 固定在北京时间凌晨 02:00 触发四象限计算。虽然能规避部分数据落盘延迟，但会让 `computed_at` 与用户理解的收盘日期天然错位，也不利于后续依赖“当天收盘后结果”的其他任务链路。

**决策**:
1. 四象限自动计算改为配置驱动，默认北京时间每日 20:00 执行，配置统一放入 `config.Quadrant`，并由 `main.go` 透传给 worker。
2. worker 必须按 A 股 / 中国香港两个市场分别判断是否为交易日；某一市场休市时，只跳过该市场，不连带跳过另一市场。
3. 自动任务的交易日判断与 A 股 `source_trade_date` 解析优先使用外部 benchmark 日线确认当天收盘口径，内部 closing snapshot 只作为回退来源。
4. 管理后台与用户态文案统一改为“每日 20:00 收盘后自动计算”，不再出现“凌晨定时任务”描述。

**原因**:
1. 20:00 调度更容易让 `computed_at` 与 `source_trade_date` 保持在同一自然日，减少用户和下游任务对日期口径的误解。
2. A 股与中国香港交易日不完全一致，市场维度跳过能避免在一边休市时错误触发另一边，或把两个市场的状态揉成一个结果。
3. 外部 benchmark 日线比内部 closing snapshot 更接近“当天收盘是否已经可用”的真实口径，能降低 20:00 刚触发时因为内部数据未落盘导致的错误回退。

**替代方案**:
- 继续保留凌晨 02:00：否决，会持续制造 `computed_at` 与 `source_trade_date` 的自然日错位。
- 仅把硬编码 02:00 改成 20:00：否决，缺少配置化和按市场跳过能力，后续再改时间或拆分市场会继续返工。
- 继续只让 A 股成功后再顺带触发中国香港：否决，无法表达“港股今天休市但 A 股照常运行”这类市场差异。

**风险与代价**:
- 若 20:00 时外部日线源仍未更新到当天，系统只能回退到最近交易日；因此日志和状态必须允许出现“20:00 触发但仍使用上一交易日”的情况。
- 新增配置项后，部署环境需要同步补齐 compose / env 默认值，否则会出现环境间行为不一致。
- 管理后台前端状态需要识别 `skipped`，否则运维会误以为任务未执行。

## 决策 21: 资金星图以 `/capital-map` 站内原生页接入看板

**日期**: 2026-06-28

**背景**: 独立项目 `liquidity-layer-monitor` 已实现单页「资金星系」，基于东方财富公开行情接口展示 A 股高流动性样本的 PE、成交额、板块资金流和 PoC 估值锚。需要迁入卧龙网站，并放入「看板」分组，命名为「资金星图」，位于「市场全景」之后。

**决策**:
1. 路由固定为 `/capital-map`，导航显示「资金星图」，归属「看板」分组并排在「市场全景」之后。
2. 首期仅支持 A 股，页面公开可见，权限口径与「市场行情」「市场全景」保持一致。
3. 允许引入 `echarts` 作为前端图表依赖，以保持原页面散点图、板块资金图和 PE 分箱图能力。
4. 数据源首期继续沿用东方财富公开行情接口，但外部请求必须封装在卧龙 Go 后端 `/api/capital-map`，前端不得直接请求东方财富。
5. 保持原刷新和算法口径：前端 60 秒刷新；后端 30 秒内存缓存；PE 优先使用 PE TTM，否则回退动态 PE；PoC 按 `0 < PE <= 120`、5 倍 PE 分箱、分箱成交额最大值计算。
6. UI 必须使用卧龙语义化 Tailwind token，并根据 `useTheme().resolvedTheme` 在图表配置中切换浅色/深色配色；A 股涨跌颜色继续采用红涨绿跌。

**原因**:
1. 原页面能力高度依赖东方财富的 PE、主力净流入和板块资金字段；直接换源会破坏“刷新频率、算法和功能不变”的目标。
2. 把外部源封装在 Go 后端可复用鉴权、缓存、错误处理与未来运维观测能力，并避免前端暴露第三方接口细节。
3. ECharts 能低成本保持原交互与图表表达，避免为散点 tooltip、视觉映射和双轴图重写自研图表。

**替代方案**:
- 直接 iframe 原站：否决，会割裂导航、主题、权限和运维链路，也无法统一浅色/深色适配。
- 复用 Factor Lab 离线因子数据：否决，Factor Lab 是夜间批处理，不满足 60 秒资金观察口径。
- 直接使用现有腾讯行情源：否决，现有 live 链路不完整覆盖 PE TTM、板块主力资金和 PoC 所需字段。
- 不引入 ECharts、改用自研 SVG/Canvas：否决，迁移成本高且容易偏离原功能。

**风险与代价**:
- 东方财富公开接口不是正式商用授权数据源，页面必须保留数据口径和风险说明；生产商用前需确认授权或切换授权源。
- 外部字段可能漂移，后端需集中解析并保留缓存降级；后续如页面流量上升，应演进为 worker 定时采集 + SQLite 快照。
- 新增 ECharts 会增加前端依赖和图表 chunk，需通过动态 import 限制首屏影响。

## 决策 20: 首页跟随导航升级为“AI投研工作台”信息架构

**日期**: 2026-06-27

**背景**: 主站导航已更新为「卧龙AI / 看板 / 跟踪 / 选股 / 更多」，产品能力也新增或强化了 AI分析、AI研报、AI选股、因子实验室、组合跟踪、持仓管理等入口。旧首页仍以「A股+港股 / 卧龙AI投研模型 / 100+指标」作为核心卖点，无法准确表达当前产品价值。

**决策**:
1. 首页主叙事从“量化工具集合”升级为“AI投研工作台”，强调「发现市场机会 → AI判断 → 因子验证 → 组合跟踪 → 持仓复盘」。
2. 首页三大核心卖点固定为「AI 投研闭环」「因子驱动选股」「组合跟踪与复盘」，不再把“市场覆盖”和“指标数量”作为第一层卖点。
3. 「快速上手」按用户任务组织，覆盖 AI分析、AI研报、AI选股、因子实验室、组合跟踪、持仓管理六条路径。
4. 「我们提供什么」按能力域分类展示：AI投研、市场与机会发现、选股与策略研究、跟踪与组合管理、账户服务。
5. 首页内容统一收敛到 `frontend/data/homepage.js`，`frontend/pages/index.js` 只负责渲染和登录 CTA 行为。
6. AI分析、AI研报、AI选股、因子排序、回测、模拟组合、交易信号相关入口必须保留显著风险提示，避免表达为收益承诺。

**原因**:
1. 新导航已经把「卧龙AI」作为第一入口，首页应同步强化 AI 投研能力，而不是继续突出旧的指标数量卖点。
2. 用户首次进入首页时更关心“我要完成什么任务”，任务型快速上手比旧功能平铺更容易理解。
3. 内容配置化能降低后续新增 AI回测、券商同步、更多组合能力时的改动成本。

**替代方案**:
- 只替换 Hero 文案：否决，无法解决快速上手、功能总览和教程整体过时的问题。
- 将所有功能平铺成卡片墙：否决，首页会信息过载，且无法体现新版导航层级。
- 首页内容后台化：暂缓，当前更新频率不高，前端静态配置更简单可控。

**风险与代价**:
- 首页覆盖能力更多，若分类和折叠设计不当，移动端会变长；因此移动端应保持单列、分组、Accordion。
- 部分功能仍可能是占位或分阶段能力，首页必须用状态标签标明「即将上线」「人工交付」「登录后可用」等状态。

## 决策 19: AI研报第一阶段采用“公开缩略图 + 登录预览 + 微信人工转化”轻量闭环

**日期**: 2026-06-27

**背景**: 需要在「卧龙AI」下新增「AI研报」页面，展示 A 股和中国香港股票个股 AI 深度研报样例，并引导用户付费定制。业务尚处于转化率和付费可行性验证阶段，不适合一次性建设订单、支付、额度、站内交付和自动生成系统。

**决策**:
1. 主站导航中「AI研报」位于「AI分析」下方、「AI选股」上方，路由为 `/ai/reports`。
2. 用户侧页面公开访问，但研报内容按权限分层：未登录只返回和展示缩略图；登录后通过弹窗请求预览图；用户侧不返回原图地址，也不提供下载按钮。
3. 付费、套餐购买和额度核销第一阶段全部由微信人工管理，页面只展示静态套餐和后台配置的微信二维码/微信号，不展示额度、订单或支付状态。
4. 研报元数据保持极简：`stock_name`、`symbol`、`exchange`、`source_trade_date`、三类图片 key；`id`、`created_at`、`updated_at` 由系统自动生成。
5. 套餐价格第一阶段写在前端静态配置，不新增 `ai_report_pricing_plans` 表。
6. `/admin/ai` 增加 AI研报管理和服务配置，用于维护 COS 图片 key / URL、微信二维码、交付时效和风险免责声明。
7. 页面必须显著展示合规风险提示，因为 AI研报会包含投资建议、目标位、止损位等内容。

**原因**:
1. 缩略图公开、预览登录化可以兼顾转化展示与内容保护，避免未登录用户直接浏览完整研报。
2. 弹窗预览比详情页更符合第一阶段“样例展示”定位，减少路由、SEO 和内容管理复杂度。
3. 微信人工转化能最快验证真实付费意愿，避免提前引入支付和额度状态机。
4. 极简数据模型降低 admin 维护成本，也符合当前“手动上传样例”的运营方式。
5. 风险提示前置是投资建议类页面的必要约束，不能隐藏在页脚小字中。

**替代方案**:
- 直接做站内支付和额度系统：否决，当前还未验证转化率，会引入订单、支付、核销、售后和交付状态复杂度。
- 所有用户都可查看完整长图：否决，样例内容保护不足，也削弱登录转化。
- 新增研报详情页：暂缓，第一阶段弹窗已满足预览需求；样例库规模变大后再考虑详情页。
- 套餐价格后台配置：暂缓，当前只有三档固定价格，静态配置更简单。

**风险与代价**:
- 如果 COS 对象是公网 URL，后端只能控制“不把预览图地址返回给未登录用户”，无法彻底阻止已知 URL 传播；后续如需强保护，应改为后端鉴权代理或短期签名 URL。
- 微信人工核销会带来运营台账风险，但这是第一阶段为速度做出的取舍。
- 套餐静态写死意味着调价需要发版；若价格频繁变化，再迁移到后台配置。

## 决策 18: Admin 后台改为 `/admin` 根入口下的分组路由 + 页面级按需加载

**日期**: 2026-06-16

**背景**: 原管理后台由 `frontend/pages/admin.js` 单文件承载全部面板，文件超过 3000 行，首屏会并发请求统计、AI、支付、备份、四象限、因子等多组接口。管理员即使只查看某一块，也要承担整页加载、滚动定位和移动端长页面操作成本。

**决策**:
1. 保留 `/admin` 作为总入口，但将后台拆为 `/admin`、`/admin/data`、`/admin/ai`、`/admin/ops` 四个前端路由。
2. 引入 `AdminShell` 作为统一壳层，负责 session 校验、退出登录、PC 左侧导航、移动端抽屉导航，以及旧 `?tab=` 参数到新路由的兼容跳转。
3. 各业务面板继续复用原有后端接口，不新增后端聚合 API；按页面分组组合现有请求。
4. 数据请求改为页面级按需加载，不再在总览页预取 AI、支付、备份、四象限、因子等非当前页面数据。
5. 原超长 `admin.js` 只保留总览路由入口，具体面板拆到 `frontend/components/admin/AdminSections.js`，路由与导航配置独立到 `AdminShell.js` / `navigation.js`。

**原因**:
1. 页面拆分比继续在单页内堆 tab 更容易建立稳定的信息架构，也更符合移动端操作路径。
2. 保持后端接口不变，可以把本次重构范围控制在前端，同时立刻获得可维护性和性能收益。
3. 把导航壳层、路由入口、面板实现分开后，后续新增 admin 分组时不需要再修改一个 3000+ 行巨型文件。

**替代方案**:
- 继续保留单页，仅增加页内 tab：否决，仍会首屏预拉所有数据，且移动端仍然面临超长滚动与局部状态耦合。
- 直接引入更细粒度权限模型并同时重做后台：否决，当前只有超级管理员一个角色，额外权限设计只会增加复杂度。
- 新增单一后端聚合接口供整个 admin 消费：否决，会把本已松耦合的后台能力重新绑成大接口，放大回归面。

**影响**:
- `frontend/pages/_app.js` 需要把 admin 独立布局判断扩大到所有 `/admin*` 路径。
- 管理后台测试需要从“检查单页内容”转为“检查 shell、分组路由、页面边界和按需加载约束”。
- 后续若新增 admin 分组，应优先新增独立路由和页面级请求清单，而不是回退到单页堆叠。

## 决策 16: `/live-trading` 升级为指数卡片化总览页，趋势图先走前端占位序列

**日期**: 2026-06-13

**背景**: `live-trading` 在导航重构后只保留市场指数，一期实现仅以单行文本列出 A 股与港股指数。页面信息层级过薄，既无法表达市场趋势，也不利于继续承接更多指数观察维度。当前 `/api/live/market/overview` 只稳定提供指数点位与涨跌信息，尚未提供可直接消费的指数趋势序列。

**决策**:
1. `/live-trading` 继续复用现有两路 overview 接口，不新增后端趋势接口作为本次前置条件。
2. 页面结构升级为 `Hero 总览 + 核心指数卡片 + 扩展指数观察 + 市场观察摘要` 四层 dashboard。
3. 首屏核心指数固定为 6 张：上证指数、深证成指、创业板指、恒生指数、恒生中国企业指数、恒生科技指数。
4. 扩展指数观察优先按前端可识别代码承接沪深300、科创50、上证50、中证500；若数据源未返回，则明确展示推荐补充提示，而不是静默缺失。
5. 在后端尚未提供真实趋势序列前，卡片内迷你图先用“基于最新点位与涨跌幅推导的 7 点占位序列”渲染，目标是提供趋势感与卡片视觉支撑，不把它包装成专业分时图。

**原因**:
1. 当前用户诉求是先把页面从“单薄指数列表”提升为“可读的市场总览”，前端卡片化与摘要化收益最高、回归面最小。
2. 若等待后端补齐趋势接口再改 UI，会继续拖延页面可用性；而占位趋势图足以满足当前“丰富页面、强化趋势感”的目标。
3. 把补充指数收敛在固定代码白名单里，可以避免页面因为接口返回顺序漂移而出现布局抖动。

**替代方案**:
- 先新增后端 `index-trends` 接口再动前端：否决，本次目标是快速完成页面升级，不应扩大到后端联动改造。
- 继续只展示文本列表、额外堆说明文案：否决，不能解决趋势感缺失和页面单薄问题。
- 直接接入完整专业图表组件：否决，当前首页定位是总览，不需要引入高交互成本和额外性能负担。

**影响**:
- `frontend/pages/live-trading.js` 需要抽出指数标准化、卡片分组、市场摘要与占位趋势图构造逻辑。
- 页面测试不再只校验“仍然只有市场 overview 数据”，还要覆盖核心/扩展卡片分组与新增指数映射。
- 后续若后端补充真实趋势字段，应优先替换 `buildTrendSeries(...)`，而不是重写卡片布局。

---

## 决策 15: 主站导航改为统一配置驱动的五组信息架构

---

## 决策 17: `/portfolio-tracking` 改为按市场分区的四组合平铺总览

**日期**: 2026-06-14

**背景**: `portfolio-tracking` 页面当前只有 4 个固定组合（A股-A / A股-B / 港股-A / 港股-B），但旧版仍采用“市场 tab + 组合 tab”的双层切换结构。用户在比较不同市场与不同组合时需要频繁切换，且滚动后容易丢失“当前看到的是哪个市场”的上下文。

**决策**:
1. 页面取消 `selectedExchange` / `selectedVariant` 双 tab 交互，改为 A股、港股两个 section 常驻展示。
2. 每个市场 section 固定展示 2 张组合卡片（A / B），形成“市场分区 + 组合卡片”的二维总览结构。
3. A股与港股使用不同视觉语义：A股采用暖色 accent，港股采用冷色 accent，同时在 section 和卡片头重复展示市场 badge，强化市场识别。
4. 单卡信息密度从“完整详情页”压缩为“4 个核心指标 + 3 个次级指标 + 迷你曲线 + 最多 4 条成分股 + 折叠式调仓”，优先保障四卡并排时的扫读效率，并减少额外说明文案。
5. 当单个组合缺失时保留占位卡，避免市场分区与 A/B 卡位因后端数据波动而发生布局跳动。

**原因**:
1. 该页面信息域单一，核心任务就是比较 4 个组合表现，不需要用 tab 隐藏内容。
2. 先按市场分区，再在区内横向比较 A/B，比 4 卡完全混排更符合用户认知，也更容易一眼区分 A股和港股。
3. 单卡降密度能把比较动作前置到首屏，同时保留成分股和调仓明细作为二级信息，不牺牲信息完整性。

**替代方案**:
- 保留 tab，仅增加快捷切换：否决，仍然无法解决“同屏对比 4 个组合”的核心问题。
- 直接 2x2 混排 4 张卡，不做市场 section：否决，市场边界太弱，A股/港股识别不够稳。
- 每张卡继续保留完整 7 指标 + 长图表 + 全量持仓：否决，四卡平铺下信息过载，难以快速扫读。

**影响**:
- `frontend/components/RankingPortfolioPanel.js` 需要引入市场配置、视图模型和 section/card 结构拆分，替换旧的选中态详情布局。
- 组件测试需要从校验 tab 状态切换改为校验市场分区、四卡平铺与颜色语义。
- 后续若新增市场或组合，应继续沿用“market section -> fixed card slots” 模式，而不是重新引入 tab。

## 2026-06-16 AI Picker 放开 LLM 输出 token 限制

- 决策：`callLLM` 不再设置 `max_tokens`，完全由 provider 默认策略决定输出上限；同时将 HTTP timeout 从 90s 提升至 240s。
- 原因：当候选池 60+ 条 × 每条 20+ 字段 + 每只股票要求 3-5 句中文理由时，完整 JSON 输出超过 4096 tokens，导致稳定触发 `finish_reason=length`。DeepSeek 支持 1M 上下文，provider 本身有充足能力；人为限制 `max_tokens` 是根因。
- 替代方案：收缩 schema 字段、压缩 prompt 候选数量、压缩每只股票输出约束。
- 不选原因：产品定位要求完整中文理由和全字段输出；这是一天只运行一次的低频服务，token 成本不是约束。
- 风险与代价：若 provider 仍有默认上限导致截断，错误信息现在更具可观测性（含 `completion_tokens` / `prompt_chars`），便于排查；HTTP timeout 延长增加了单次请求的最大等待时间，已在 admin 页面通过异步生成+状态查询规避用户感知。

## 2026-06-17 Admin AI 页面保留最近一次 AI Picker 完整交互详情，但不扩展为历史全量归档

- 决策：`/admin/ai` 的 AI Picker 运维面板新增“最近一次生成详情”，只保留并展示最近一次生成对应的 `system prompt`、`user prompt`、provider 返回的 `reasoning` 字段（若有）与 AI 原始返回内容；日志列表从 20 条缩减为 10 条，所有时间统一按北京时间显示。
- 原因：当前核心诉求是让管理员能快速复盘“最近一场生成到底发了什么、模型回了什么”，不需要把每一轮完整 prompt/response 都长期沉淀成高体量审计库。最近一次详情 + 最近 10 条摘要日志能覆盖日常排障，同时控制存储规模和页面噪音。
- 替代方案：
  - 继续只展示摘要日志，不保存完整 prompt/response：否决，无法满足排障和复盘需求。
  - 为每条历史日志都保存并展示完整 prompt/response：否决，当前 admin 主要是低频运维面板，全量历史会快速放大存储和 UI 复杂度。
  - 直接展示 provider 的原始完整响应 JSON：否决，provider 返回结构不稳定，前端应消费收敛后的字段。
- 不选原因：本次目标是补齐最近一次运行的可观测性，而不是建设通用审计系统；应优先保持数据模型简单、页面读取成本低。
- 风险与代价：如果 provider 不返回 `reasoning` 字段，页面只能展示“未返回 reasoning”；当前 trace 与 generate log 通过最新一条关联，若后续需要审计历史所有 prompt/response，需要再扩展保留策略与检索界面。

## 2026-06-15 AI Picker 技术快照软增强
- 决策：AI Picker 中技术快照从“硬门槛”改为“软增强”；候选股缺少技术快照时仍保留，只将相关技术字段置空，并在 prompt 中显式标注技术信息缺失。
- 原因：技术快照依赖外部日线接口，单点失败不应导致整个候选池被清空；当前真实主链路是因子召回，技术面只是增强信息。
- 替代方案：继续把技术快照作为硬过滤条件；技术快照失败时直接报错阻断生成。
- 不选原因：硬过滤会把“增强失败”误表现为“候选池为空”，导致生成可用性过低，也不符合因子优先的产品定位。
- 风险与代价：部分推荐会缺少技术面细节，需通过 prompt 明示并在理由中说明“技术面信息不足”；同时仍需保留日志以便后续排查技术快照链路。

## 2026-06-14 AI Picker P0
- 决策：A 股 AI 选股采用 `factorlab` 候选池 + LLM 二次决策，而不是纯 prompt 直出。
- 原因：减少幻觉，允许理由中引用真实因子分值，并复用现有日更因子资产。
- 替代方案：纯 prompt 选股、港股同时接入。
- 不选原因：纯 prompt 缺乏真实数据约束；港股因子链路尚未就绪。
- 风险与代价：结果质量仍依赖 prompt 与 post-validation；每日结果表当前只保存最终 JSON，不保存更细中间候选日志。

## 决策 27: 模拟组合重启跟踪统一使用 start_signal_date + 严格共同日期

**日期**: 2026-07-01

**背景**: 4 个新口径模拟组合已经持续运行，Admin 需要支持“停掉当前历史结果，并从管理员指定日期重新开始跟踪”。该操作会影响全部 4 个组合，且必须在应用前判断指定日期是否具备可用数据。

**决策**:
1. 管理员选择的日期定义为 `start_signal_date`，即榜单信号日 / 收盘日，不是买入交易日。
2. 采用严格共同日期：A 股与港股必须在同一个 `start_signal_date` 都有排行榜快照，4 个 active 模拟组合才能一起应用。
3. 不支持单组合单独指定开始日期，避免 `/portfolio-tracking` 四卡并排时起始窗口不同导致收益、回撤、波动率不可比。
4. 新增 `SimPortfolioTrackingStartService`，专门负责全局起点 status、preview 与 apply；底层重算仍复用既有模拟组合事实表计算链路。
5. apply 前必须重新执行 preview；预检不通过时拒绝清空/重算事实表。
6. 起点配置与 apply job 持久化到 `sim_portfolio_tracking_config` / `sim_portfolio_tracking_jobs`，用于 Admin 展示和审计。

**原因**:
1. 模拟组合因果链是 `signal_date -> next trade_date open -> trade_date close valuation`；让管理员直接选择买入日会引入“前一交易日”推断，容易违反交易日历规则。
2. 严格共同日期牺牲少量可选日期，但保证 4 个组合的收益窗口一致，符合“所有组合同时生效”的产品约束。
3. 预检先暴露缺快照、成分股不足、缺开盘价、缺收盘价等阻断原因，避免清空事实表后才失败。
4. 独立 service 能保持职责清晰：普通 sync 负责推进最新事实表，tracking start apply 负责危险的全局重启。

**替代方案**:
- 使用 `start_trade_date`：否决。需要按市场反推前一信号日，A 股/港股交易日不同，语义复杂且易错。
- 市场内自动顺延到最近可用日期：暂不采用。会导致 A 股和港股真实起点不同，不符合“同时生效”。
- 直接复用旧「从头重算全部组合」按钮加一个 from_date：否决。接口无法表达预检、全局起点配置和任务审计。
- 支持单组合起点：否决。会破坏 4 个组合横向对比口径。

**风险与代价**:
- 严格共同日期可能让部分 A/H 休市错开的日期不可选，Admin 需要换一个共同有数据的日期。
- apply 当前为同步重算；如果未来历史窗口变长、耗时增加，应演进为异步 job + 轮询进度。
- apply 会清空并重建 4 个组合事实表，必须保留二次确认、后端锁和 apply 前再次预检。

## 决策 29: 镜像构建缓存从 TCR registry cache 切换为 GitHub Actions 本地缓存

**日期**: 2026-07-02

**背景**: 现有 `publish_images` job 在同一条 `docker buildx build --push` 链路里同时执行最终镜像推送和 `type=registry` cache export。backend / frontend / quant 三个镜像都要把 BuildKit cache 推到 TCR 的 `:buildcache` 标签，导致每次构建都被跨境 cache 上传拖慢；而 deploy 实际只依赖最终镜像，不依赖 cache export 成功。

**决策**:
1. Phase 1 保持 `publish_images` 为单个串行 job，不拆并行 job，也不调整 deploy 拓扑。
2. 三个镜像统一移除 `type=registry` 的 `--cache-from/--cache-to`，改用 `type=local` Buildx cache，并通过 `actions/cache@v4` 持久化到 GitHub Actions Cache。
3. backend / frontend / quant 各自使用独立缓存目录和独立 key，禁止共享 cache 目录。
4. 每个镜像构建时使用 `cache-from=type=local,src=...`，输出到 `cache-to=type=local,dest=...-new,mode=max`，构建成功后再用目录替换更新缓存。
5. cache key 仅绑定 Dockerfile 与依赖定义文件，避免把频繁变动的业务源码纳入 key，优先复用依赖层。

**原因**:
1. 最慢的部分是 registry cache export，而不是最终镜像 push；去掉远端 cache 上传可以直接缩短主链路尾部耗时。
2. GitHub Actions Cache 位于 GitHub 侧网络，稳定性通常高于跨境推送到 TCR 的 build cache。
3. 独立缓存目录与独立 key 可以为后续 Phase 2 并行化铺路，避免多个镜像之间 cache 污染。
4. 先只做缓存机制替换，能把变更面控制在最小范围，便于验证收益与稳定性。

**替代方案**:
- 继续保留 TCR registry cache，仅增加重试：否决。根因是远端 cache export 链路过慢，不是单纯偶发失败。
- 一次性同时做并行 job、显式产物摘要和 deploy 校验：暂缓。当前先验证缓存机制替换收益，降低首轮改造风险。
- 三个镜像共用一个本地 buildx cache 目录：否决。会引入缓存污染、key 不稳定和后续并行写覆盖问题。

**风险与代价**:
- `actions/cache` 仍受 GitHub 缓存容量和命中策略影响，首次构建或 key 变更时不会提速。
- `mode=max` 可能放大缓存体积；若后续命中率不理想或缓存过大，再考虑降为 `mode=min`。
- 当前仍是单 job 串行构建，整体时长上限尚受最慢镜像累加影响；并行收益留到 Phase 2。

## 决策 30: 镜像构建拆为三个并行 job，deploy 统一等待全部完成

**日期**: 2026-07-02

**背景**: Phase 1 已将 Buildx cache 从 TCR registry cache 切到 GitHub Actions Cache，但 `backend`、`frontend`、`quant` 仍在同一个 `publish_images` job 中串行执行。即便单镜像尾部耗时已下降，总耗时仍然等于三个镜像构建时间之和，最慢的 frontend 仍会阻塞 backend / quant 的最终完成。

**决策**:
1. 删除单一 `publish_images` job，改为 `build_backend`、`build_frontend`、`build_quant` 三个并行 job。
2. 三个 build job 全部依赖语言级 `build` 检查，保证代码检查仍先于镜像发布。
3. 每个 build job 都必须独立完成：解析镜像变量、生成 `image_repo` / `image_tag` / `image_ref` outputs、登录 TCR、恢复本镜像专属 cache、build 并 push 最终镜像。
4. `deploy` 必须显式 `needs` 三个 build job 全部成功后再启动，不允许依赖单一汇总 job。
5. Phase 2 先只输出镜像摘要，不新增 digest 校验、manifest 校验或服务器侧显式拉取前检查；这些留到 Phase 3。

**原因**:
1. 并行后总耗时将从三镜像累加，下降为近似最慢单镜像耗时，更符合当前优化目标。
2. 每个 job 独立 restore 自己的 cache path，可以天然避免后续并行写冲突。
3. 每个 job 直接输出自身镜像引用，为后续 Phase 3 增加存在性校验和更严格的 deploy 输入打基础。
4. deploy 明确依赖三个 job，可避免某个镜像未完成就误触发部署。

**替代方案**:
- 保留单一 `publish_images` job，仅在内部后台并发 shell 命令：否决。日志、失败定位、缓存隔离和依赖表达都更差。
- 先做并行 job，同时把 deploy 改为按 `image_ref` 精确部署和远端校验：暂缓。当前只推进 Phase 2，避免一次性扩大变更面。
- 让 deploy 继续只依赖一个中间汇总 job：否决。会弱化“全部构建成功后才部署”的语义，不利于后续收口。

**风险与代价**:
- 三个 job 会重复执行 checkout、buildx setup、TCR login，Runner 总消耗会增加，但换来更低 wall-clock 时间。
- 当前输出的是 `image_ref` 而非 digest，仍无法彻底防止 tag 被误覆盖；该问题留待 Phase 3 解决。
- deploy 仍按统一 `IMAGE_TAG` 部署，因此当前主要收益是 CI 并行耗时下降，而不是发布一致性语义增强。

## 决策 28: 个股详情页采用“首屏 AI 决策概览 + 分域 Tab”信息架构

**日期**: 2026-07-01

**背景**: `/live-trading/[symbol]` 个股详情页承载实时行情、AI 分析、新闻、基础面、技术指标、持仓和交易信号等大量内容。旧版全部纵向堆叠，页面过长，尤其在移动端不利于用户快速完成「看行情」和「AI 判断」。用户确认核心任务优先级为「看行情 / AI 判断」优先，「管理持仓」其次；同时 AI 分析是商业转化核心入口，必须常驻首屏。

**决策**:
1. 个股详情页采用「首屏决策概览 + 分域 Tab」而不是继续纯长页面。
2. PC 端固定五个 Tab：概览、走势、技术、基本面、持仓 & 提醒。（2026-08-15 修订：「新闻与公告」Tab 已下线，聚焦抄作业核心功能；新闻数据仍通过 `/api/live/symbols/{symbol}/news*` 供 AI 分析上下文使用。）
3. 移动端固定四组入口：概览、走势、分析、持仓；其中「分析」内部再二级切换技术、基本面。
4. AI 分析入口必须在首屏概览中常驻；AI 分析历史也放在概览中，替代原「查看最近观点」按钮。
5. 持仓与交易信号降为次级任务，完整交易信号配置归入「持仓 & 提醒」Tab；未登录用户看到登录引导而不是空白。
6. 本阶段仅前端 UI 重构，不新增后端聚合接口；Tab 状态通过 URL query `tab` 表达，非法值回退 `overview`。

**原因**:
1. 首屏概览能保留用户进入个股页后的即时判断能力，不会把关键结论藏进 Tab。
2. 分域 Tab 能显著降低页面长度，并让用户按任务进入对应信息域。
3. PC 与移动端访问占比接近，PC 保留更完整的信息架构，移动端压缩主入口更符合单列浏览。
4. AI 是商业核心入口，若只放在某个 Tab 内会降低转化和发现效率。
5. 不改后端接口可以控制回归面，先解决 UI 信息过载问题。

**替代方案**:
- 继续纯长页面，仅优化卡片样式：否决，无法解决页面过长和信息优先级不清。
- 简单把所有 section 平铺进 Tab：否决，首屏会缺少即时决策摘要，用户必须先选择 Tab。
- 拆成多个子路由：暂不采用，会扩大路由、权限、状态恢复和回归测试成本。
- PC/移动完全相同六 Tab：否决，移动端主导航过长，影响扫读和点击效率。新闻独立成组后，移动端仍保持短标签入口。

**风险与代价**:
- 当前仍由 `frontend/pages/live-trading/[symbol].js` 承载多个 Tab 的具体内容，文件仍偏大；后续应继续拆出 Tab Panel 组件。
- 本阶段只是条件渲染分域，部分数据请求仍沿用原页面加载节奏；如性能压力继续存在，应再做按 Tab 可见性延迟加载。
- 移动端「分析」组增加二级切换，需要保持技术/基本面/AI 三个子域都可达。

## 决策 31: 镜像发布改为本地 shell 入口，Phase 2 先落地制品构建与 TCR 上传

**日期**: 2026-07-02

**背景**: 当前 GitHub Actions 已演进到并行构建 `backend`、`frontend`、`quant` 三个镜像，但构建与 push 仍发生在 CI 内。用户进一步确认要将“构建 + push TCR”完全迁移到本地执行，Actions 后续只负责部署指定 tag。Phase 2 仅实现本地发布脚本，不处理 deploy workflow 重构与回滚治理。

**决策**:
1. 新增统一入口 `ops/local/release.sh`，承担本地镜像构建与可选 push。
2. 支持服务固定为 `backend`、`frontend`、`quant`，并要求通过 `--services` 或 `--all` 显式指定目标。
3. tag 默认规则为 `release-时间-shortsha`，允许手工通过 `--tag` 覆盖。
4. 仓库映射、Dockerfile、build context 下沉到 `ops/local/release.config.sh`，避免主脚本写死服务拓扑。
5. 每次执行都输出 `.release/manifests/<tag>.json`，作为后续 deploy 与审计输入；`.release/` 不纳入 git。
6. Phase 2 保持串行构建，`--parallel` 只保留参数位，不真正启用并行。

**原因**:
1. 本地先构建/上传，可以先独立验证制品生成，不必等 deploy 流程一起联调。
2. 将制品层与部署层拆开后，后续 Actions 可收敛为纯部署任务，职责边界更清晰。
3. 配置化服务映射为后续新增服务或调整 repo 命名留下扩展点。
4. manifest 让本地发布结果可追踪，为后续远端镜像校验、digest 闭环与回滚能力打基础。

**替代方案**:
- 继续只在 CI 中构建 push，开发者本地不提供入口：否决。不满足“先独立测试本地构建和上传 TCR”的目标。
- 直接把本地脚本做成并行 build/push：暂缓。当前优先降低脚本复杂度，先稳定跑通 Phase 2。
- 不输出 manifest，只在控制台打印结果：否决。后续 deploy、审计与故障排查都需要结构化产物。

**风险与代价**:
- 本地机器环境差异会导致构建耗时和稳定性波动，后续可能仍需专用 builder。
- 当前未做远端 tag 存在性与 digest 校验，仍存在“push 成功但部署未校验闭环”的风险。
- `.release/` 为本地产物目录，需要明确忽略并避免误提交。

## D-006: 模拟组合 v2 改为 Calendar-driven Pipeline（2026-07-04）

**决策内容**: 新模拟组合链路统一命名为 `Sim Portfolio v2`，以市场交易日历驱动 pipeline，不再使用“下一条排行榜快照”推断下一交易日，不再通过旧 `quadrant_ranking_portfolio_market_prices` 作为补价和推进事实表的核心状态。

**原因**:
- A 股与港股交易日历不同，休市日不能被误判为缺开盘价或缺收盘价。
- 某交易日四象限或排行榜快照失败时，旧链路会从已有快照序列中静默跳过该日，无法显式识别缺口。
- 旧 Admin 的补价/同步按钮只能局部修补字段，不能追踪四象限、信号、选股、价格需求、事实表和验证的完整链路。

**替代方案**:
- 继续修补旧 `sim_portfolio_service` 与补价按钮：不选，因为核心抽象错误，技术债会继续扩大。
- 缺信号时沿用上一日持仓保持曲线连续：不选，因为会污染正式收益口径。

**约束**:
- 严格模式：缺信号、缺价格或验证失败时，不生成该日正式收益。
- 休市日必须标记为 skipped，不得产生价格需求。
- `entry_open` 与 `valuation_close` 必须精确匹配目标交易日。
- v2 不迁移旧模拟组合历史数据，从最新完整交易日重新启动。

## D-007: 模拟组合 v2 Admin 采用双市场日历驾驶舱与市场独立起点（2026-07-04）

**决策内容**: Admin 模拟组合 v2 运维界面从阶段表格继续演进为双市场日历驾驶舱：A 股和港股分别展示月历；每个市场内部展示组合 A/B 状态；开始信号日改为市场级配置，A 股和港股可独立启动，但同一市场内 A/B 必须同一起点。

**原因**:
- A 股与港股交易日历不同，放在同一个日历格子中会混淆交易/休市/blocked 状态。
- 用户比较口径是市场内 A/B 对比，而不是 A 股组合与港股组合横向比较，因此市场独立起点更符合产品认知。
- 日历视图能比阶段表格更直观暴露“哪一天断了、哪个组合断了、缺什么数据”。

**替代方案**:
- 单一全局日历 + A/H 混合状态：不选，信息密度过高且休市差异会造成视觉噪音。
- 全局共同开始信号日：不选，会让港股休市或缺数据不必要地阻塞 A 股启动。
- 单组合独立起点：不选，会破坏同一市场内组合 A/B 的收益窗口可比性。

**风险与代价**:
- 市场级起点会让 A 股和港股收益窗口不同，Public 页面需要展示市场级起点以避免误解。
- 当前 apply 基础实现仍是同步重建；后续应演进为异步 job + staging + publish，避免长请求和半成品发布风险。
- 缺四象限和缺价格的修复按钮需要继续落地上游重建/价格修复能力，否则日历只能发现问题，不能完全闭环。

## D-008: 模拟组合 v2 缺口修复采用显式三层动作与审计（2026-07-04）

**决策内容**: `missing_signal` 通过 Admin 指定 `market + source_trade_date` 触发上游四象限重建；缺价格按“重新解析价格 → 重拉该日缺失历史日线 → 人工覆盖价格”三层动作处理。人工覆盖必须填写原因和证据并写入审计，修复动作不得直接修改 verified facts。

**原因**:
- `missing_signal` 是上游四象限/排行榜缺口，不能由模拟组合链路自行跳过或伪造信号。
- 价格缺失优先应由标准价格源恢复，人工覆盖只能作为最后手段。
- 模拟组合收益是用户高关注正式口径，任何人工改价都必须可追溯。

**替代方案**:
- 一个“自动修复全部”按钮：不选，无法区分缺信号、缺开盘价、缺收盘价和人工覆盖风险。
- 人工覆盖后直接改 facts：不选，会绕过 pipeline verification，造成收益来源不可解释。
- 缺价格时用最近价或收盘价兜底：不选，会污染 T+1 开盘建仓和收盘估值口径。

**约束**:
- 修复按钮只能改变上游快照、price requirements 或 override/audit 表。
- 修复完成后必须重新运行对应市场/日期 pipeline，正式收益只能由 pipeline facts 阶段生成。
- 人工覆盖必须包含 `reason`、`evidence`、`operator` 和 `confirm=true`。

## 决策 32: SQLite 连接池扩容 + VACUUM 独立连接 + HTTP 显式超时 + 异步任务限时

**日期**: 2026-07-08

**背景**: 生产环境发生 Cloudflare 524 超时事故。根因是 `MaxOpenConns=1` 的 SQLite 连接池在四象限计算完成后触发的 backup `VACUUM INTO`（791MB 数据库，耗时 30s+）中被长时间独占，所有 API 查询排队等待 ~300s 后被 Cloudflare 边缘超时。事故窗口（UTC 12:55~13:02 = 北京时间 20:55~21:02）与 `QUADRANT_COMPUTE_HOUR=20` 吻合：quadrant 计算完成后异步触发 backup，backup 的 VACUUM 饿死连接池。

**决策**:
1. **VACUUM 独立连接**（`store/backup/service.go`）：`hotBackupPumpkin` 不再复用 `s.db`，而是通过 `gorm.Open(sqlite.Open(...))` 创建独立连接执行 `VACUUM INTO`，完成后关闭。确保 VACUUM 的排他锁不占用主连接池。
2. **连接池扩容**（`store/gorm.go`）：`MaxOpenConns` 1→4，`MaxIdleConns` 1→2，`busy_timeout` 5s→15s。SQLite WAL 模式支持并发读 + 单写，4 连接池允许读查询在写事务进行时继续执行。
3. **HTTP Server 显式超时**（`main.go`）：从 `http.ListenAndServe` 改为 `http.Server{}` 显式设置 `ReadHeaderTimeout=10s`、`ReadTimeout=30s`、`WriteTimeout=120s`、`IdleTimeout=120s`。防止被阻塞的请求无限期占用 goroutine。
4. **异步任务限时**（`store/quadrant/service.go`）：异步 goroutine 的 context 从 `context.Background()` 改为 `context.WithTimeout(context.Background(), 10*time.Minute)`，确保异步任务不会因外部依赖故障而无限运行。

**原因**:
1. VACUUM 是 SQLite 全库排他操作，在 791MB 数据库上耗时 30s+，绝不能在共享连接上执行。
2. `MaxOpenConns=1` 对 WAL 模式过于保守——WAL 允许并发读，单连接池会导致所有读查询被写操作串行化。
3. 无超时的 HTTP Server 在连接池被饿死时会产生无限堆积的 goroutine，加剧问题而非自愈。
4. 脱离请求 context 的异步任务如果没有自己的超时，可能因外部依赖（如价格解析 API）卡死而永久占用资源。

**替代方案**:
- 将 backup 改为离线模式（先停服再复制 DB 文件）：否决，会导致服务停机。
- 切换到 PostgreSQL：暂缓，当前数据量和写入频率不需要，且迁移成本高。
- 用 `.backup()` 命令替代 `VACUUM INTO`：否决，`.backup()` 会复制 WAL 依赖文件，不如 `VACUUM INTO` 产出干净的独立 DB 文件。
- `MaxOpenConns` 设更高（如 10）：否决，SQLite 本质单写，过高连接数会增加 `SQLITE_BUSY` 概率，4 是 WAL 模式的合理平衡点。

**风险与代价**:
- `MaxOpenConns=4` 在极端并发写入时可能产生更多 `SQLITE_BUSY`，但 `busy_timeout=15s` 提供了足够重试窗口。
- VACUUM 独立连接会短暂持有 DB 级排他锁，但与主连接池隔离后不会导致 API 请求排队——VACUUM 期间主连接池的读请求仍可正常执行（WAL 快照隔离）。
- HTTP WriteTimeout=120s 对极慢的 admin 操作可能不够，但这类操作已改为异步，120s 足够覆盖所有正常同步请求。
- 异步任务 10 分钟超时对极端情况可能不够，但比无限等待更安全；超时后日志可追踪。

**测试**: backup 模块全部测试通过；quadrant 模块 3 个测试失败为修改前已存在的异步竞态问题（goroutine 未执行完即查询 DB），非本次引入。

## D-021: Quant 外部数据源统一收敛到 Data Source Gateway

**日期**: 2026-07-11

**决策**:
- quant 新增 `quant/data_sources/` 作为 Tencent、EastMoney、AkShare 等外部数据源的统一入口。
- 数据源优先级按 capability + market 在 `policy.py` 中用代码常量维护，第一期不引入 env、数据库配置或 Admin 可编辑配置。
- Provider 能力通过 `registry.py` 声明；manager 遇到不支持当前 market/capability 的 provider 必须 skip 并记录 trace。
- Provider adapter 只负责取数；normalizer 负责字段归一；validator 负责数据有效性；manager 负责 fallback 和 partial response。
- 价格类数据（如 `daily_bars` / `index_bars`）在传入 `target_trade_date` 时必须精确命中目标交易日，不允许用其他日期价格兜底。

**原因**:
1. 项目已有 akshare、东方财富、腾讯等多个外部源，调用和降级逻辑散落在四象限、因子、基础面、资金星图等模块中，继续扩展会导致重复 fallback 和字段口径漂移。
2. 数据源优先级是低频变更，代码常量 + 测试比 env / Admin 可编辑配置更简单、更可审查，也避免运维面变重。
3. 历史问题表明“请求成功”不等于“字段可用”；统一 validator 能在入口处防止空字段、错误日期和无效价格被当作成功。
4. backend 应保持纯粹，未来资金星图等行情计算应迁移到 quant，backend 只做 API 代理、鉴权和缓存。

**替代方案**:
- 使用 env 配置 provider 顺序：否决。变更低频且配置和 normalizer/validator 兼容性强相关，env 会增加部署和排障复杂度。
- 使用数据库或 Admin 可编辑配置：否决。第一期会引入权限、审计、热更新和误操作风险，收益不足。
- 独立 data-service 微服务：暂缓。当前项目规模用 quant 内部模块即可，独立服务会增加部署复杂度。
- 继续各业务模块自行 fallback：否决。会扩大重复逻辑，且无法形成统一 trace 和健康观测。

**风险与代价**:
- 迁移期间会短期存在旧直连 provider 和新 Gateway 双轨，后续每迁移一个 capability 应逐步删除旧路径。
- Policy 常量变更需要发版，不能热调整；这是第一期为降低运维复杂度接受的代价。
- Gateway 返回 partial 数据时，上层业务必须检查 `ok` / `partial` / `trace` / `errors`，不能默认当作完整成功。
- 对结构化离线表（如 Phase0 `financials` / `dividends`），迁移初期允许 Gateway 复用既有抓取/解析函数生成最终行结构，只把 provider 顺序、fallback、trace 收敛到 Gateway；待 capability 稳定后再继续拆分成独立 adapter + normalizer。

## D-022: 资金星图迁移为 backend proxy → quant 计算链路

**日期**: 2026-07-13

**决策**:
- 资金星图的公开 API 入口仍保留 backend `/api/capital-map`，避免前端路由和缓存口径变化。
- backend `/api/capital-map` 只负责调用 quant `/api/capital-map`、30 秒内存缓存和 stale 降级，不再直接调用东方财富或承载资金星图算法。
- quant 新增 `capital_map` 模块承载字段归一、PE 选择、成交额排序、PoC 分箱、板块资金排序和 payload 构造。
- Data Source Gateway 新增 `capital_map` capability，第一期策略固定为 A 股 `ASHARE` 使用 EastMoney provider。

**原因**:
1. 符合 D-021：backend 保持纯粹，只调用 quant API，不理解底层 provider 字段。
2. 资金星图与四象限、因子等同属行情/量化计算能力，应收敛到 quant，避免 Go backend 与 Python quant 重复维护 provider 字段解析。
3. 保留 backend proxy 和缓存可以不影响前端调用路径，并继续提供 stale 降级保护。
4. 第一阶段仅接入 A 股 + EastMoney，避免为尚未支持的港股/美股提前扩展复杂策略。

**替代方案**:
- 继续由 backend 直接请求东方财富：否决，会与 Data Source Gateway 统一出入口目标冲突，且 provider 字段解析继续分散。
- 前端直接请求 quant：否决，会绕过 backend 的公开 API、缓存、统一错误处理和未来鉴权/观测入口。
- 一次性引入 DB 快照或 Admin 可配置数据源：暂缓。当前只做最小迁移闭环，快照和 Admin 健康区块留到后续阶段。

**风险与代价**:
- backend 到 quant 增加一次内部 HTTP 调用；通过 30 秒缓存降低请求放大。
- quant 进程需要承担资金星图外部请求耗时；若东方财富不可用，backend 只能返回 stale 或错误。
- backend 旧 `capitalmap.Service` 与 EastMoney client 暂时作为遗留代码存在，后续确认稳定后可删除，避免一次迁移扩大回归面。

## D-023: A 股四象限股票池预过滤（ST/停牌/北交所）

**日期**: 2026-07-14

**背景**: A 股四象限计算原本对全市场快照只做 `price > 0` 的数据质量过滤，ST、停牌、北交所股票全部进入横截面排名。这些股票数据质量差、流动性异常，影响四象限打分的稳定性和可解释性。用户要求在发起任何数据源请求之前先过滤掉这三类股票。

**决策**:
1. 新增独立模块 `quant/screener/universe_filter.py`，提供三个纯函数 + 一个组合入口：`is_bse_code`/`is_st_name`/`is_suspended_row`/`filter_a_share_universe`。
2. 过滤逻辑放在 `quadrant.py` 的 Step 1.5（快照之后、拉日线之前），不下沉到 `scanner.py`（通用快照层应保持中立）。
3. 全部判断只读取已拉取的快照本地字段（`code`/`name`/`volume`），不发起任何网络请求，与 baostock 可用性完全解耦。
4. 北交所判定复用 Factor Lab `classify_board()` 的 BJ 分支：`code.startswith(("8", "4", "920"))`。
5. ST 判定用子串匹配：`"ST" in name.upper().replace(" ", "")`，天然覆盖 `*ST`/`SST`/`S*ST`；额外排除名称含"退"字的退市整理期股票。
6. 停牌代理用 `volume <= 0`；保守原则：字段缺失（None/NaN）时不过滤，宁可漏过滤不可误伤。
7. 过滤比例超过 15% 时记录 ERROR 告警但不阻断流程，防上游数据异常导致大批误伤。
8. 被过滤的股票不参与四象限打分，但 `FilterStats` 写入计算报告和进度上报，供 Admin 监控。

**原因**:
1. 过滤所需信号全部可从已拉取的快照本地得出，不需要新增数据源请求。
2. 独立模块便于未来被选股器等其他业务场景选择性复用，也便于单独写单元测试。
3. 与 baostock 解耦是硬约束：过滤发生在 baostock 调用之前，baostock 不可用时过滤照常工作。
4. 复用 Factor Lab 北交所判定规则避免系统内出现两套定义。

**替代方案**:
- 用 baostock `query_all_stock()` 的 `tradeStatus` 做停牌判断：否决，会让过滤依赖 baostock 可用性。
- 直接在 `quadrant.py` 内写 pandas 过滤代码：否决，口径散落在编排逻辑里不利维护和收敛。
- 设计更严谨的正则或引入交易所公开代码段清单判北交所：否决，过度设计。

**风险与代价**:
- `name`/`volume` 字段解析异常可能误伤正常股票。缓解：保守原则 + `MAX_FILTER_RATIO` 熔断 + 上线前离线校验。
- 过滤模块可独立于 baostock 先行灰度上线，降低整体上线风险。

## D-024: BaoStock 设为 A 股四象限第一优先级数据源 + 全局配额守卫

**日期**: 2026-07-14

**背景**: 四象限 A 股日线数据源原为 `tencent → eastmoney → akshare`。baostock 提供权威的 A 股 EOD 日线（含 `isST`/`peTTM`/`turn` 等字段），适合作为第一优先级。baostock 配额为 50000 次/日（单 IP 限制，不分账号），仓库内有多处裸调用 baostock 的代码路径共享同一份 IP 配额。

**决策**:
1. 新增 `quant/data_sources/providers/baostock.py` 作为 A 股 `DAILY_BARS` 的第一优先级 provider。
2. `policy.py` 中 A 股 `DAILY_BARS` 的 provider 顺序改为 `["baostock", "tencent", "eastmoney", "akshare"]`。港股不动。
3. baostock 登录态采用长连接单例：进程启动时 `bs.login()` 一次，全程复用 session。
4. baostock 请求串行执行（非线程安全），通过 `threading.Lock` 保证。
5. 新增 `GlobalBaostockQuotaGuard` 服务级全局单例（`quant/data_sources/quota/baostock_quota.py`）：
   - SQLite 落盘跨进程共享同一份配额账本。
   - 所有 baostock 调用方必须经过 `try_acquire(cost, caller)` 检查。
   - `caller` 维度归因统计（不按 caller 分割配额，只做统计标注）。
   - 用量达到 90%（45000 次）自动标记黑名单，当天剩余请求拒绝。
   - 异常时放行（宁可多请求也不因配额守卫 bug 阻塞业务）。
6. `backfill_factor_lab_phase0.py` 的行业回填分支接入 `GlobalBaostockQuotaGuard.try_acquire(caller="factor_lab_industry_backfill")`。
7. `/api/data-sources/health` 追加 `baostock_quota` 字段，供 Admin 展示配额用量和 caller 归因。
8. 利润增速（YOYNI）默认关闭，`ENABLE_BAOSTOCK_GROWTH` 开关保留但默认 `False`。
9. `output/` 下的一次性 baostock 脚本本次不改代码接入配额守卫，作为已知过渡期风险接受。

**原因**:
1. 单 IP 限额意味着配额不是四象限一个任务的私有资源，必须是服务级全局账本，否则会出现"账本外支出"导致 IP 被拉黑却毫无察觉。
2. 长连接单例避免每次请求重复登录/登出，降低 baostock 服务端压力和自身延迟。
3. 串行执行是 baostock API 线程安全约束的必然选择。
4. 过滤模块（D-023）会减少 `all_codes` 数量，直接降低 baostock 请求量，两者协同降低配额压力。

**替代方案**:
- 配额守卫只在四象限内部维护私有计数器：否决，无法覆盖 Factor Lab 等其他裸调用路径。
- 把所有 `output/` 脚本也接入配额守卫：否决，改动面过大且这些脚本使用频率极低，作为已知风险接受更合适。
- baostock 登录态每次请求 login/logout：否决，增加延迟和服务端压力。

**风险与代价**:
- baostock 串行请求可能拖慢全量计算耗时。缓解：过滤模块减少请求量 + `MAX_WORKERS=3` 对后续 provider 的并发仍有效。
- `GlobalBaostockQuotaGuard` 多调用方并发写入需保证原子性。缓解：SQLite `BEGIN IMMEDIATE` 事务 + `busy_timeout`。
- 建议过滤模块先于 baostock provider 独立灰度上线，两者解耦上线互不阻塞。

## D-025: 新闻透视以 backend proxy → quant 计算链路接入看板

**日期**: 2026-07-21

**背景**: 用户已有本地「新闻K线透视」demo，希望迁入卧龙网站「看板」分组，作为用户搜索股票后查看新闻/公告催化剂与前复权 K 线关系的独立页面。

**决策**:
1. 路由固定为 `/news-kline`，导航显示「新闻透视」，归属「看板」分组并排在「资金星图」之后。
2. 首期通过 `/api/search` 的股票搜索表选择 A 股/中国香港股票，不新增前端直连外部搜索源。
3. 公开 API 固定为 backend `GET /api/live/news-kline?symbol=&days=&pages=&force=`，backend 只做代理、30 分钟内存缓存和 stale 降级。
4. quant 新增 `data/news_kline.py` 与 `/api/news-kline/{symbol}`，负责腾讯前复权日 K、腾讯 `news/info/search` 公告(type=0)+新闻(type=2)、事件分类、前后收益和解释力统计。
5. A 股和中国香港股票 K 线均使用前复权口径；用户展示日期以 K 线最新交易日作为 `source_trade_date`。
6. 研报首期不纳入。`研报评级` 作为分类能力保留，但空分类必须在前端隐藏；二期再评估研报源。
7. 超时与降级参数：backend quant proxy HTTP timeout 18s；quant 外部 HTTP 单次 5s、最多 2 次尝试；缓存 TTL 30 分钟。
8. 前端使用 ECharts 动态加载和 `useTheme().resolvedTheme` 配色，遵守语义化 Tailwind token 与红涨绿跌口径。

**原因**:
1. 与资金星图一致，backend 保持纯粹，业务计算和外部源字段解析收敛到 quant。
2. 腾讯 `news/info/search` 单只股票默认可拉更深历史（按页），比现有 akshare 新闻浅历史更适合解释力统计。
3. 研报数据源尚未在平台内形成稳定授权/数据口径，首期暂缓能降低上线风险。
4. 30 分钟缓存 + stale 降级适合低频研究型页面，避免用户搜索造成外部源放大。

**替代方案**:
- 复用现有 `/api/live/symbols/{symbol}/news` 的 akshare 新闻链路：未选，历史覆盖偏浅，解释力统计样本不足。
- 直接在 Go backend 里实现分类和收益计算：未选，会把行情计算逻辑从 quant 分散到 backend。
- 前端直接请求腾讯接口：否决，会绕过 backend 缓存、错误处理和未来鉴权/观测入口。
- 首期纳入研报：暂缓，研报源和授权口径需单独评估。

**风险与代价**:
- 腾讯公开接口不是正式商用授权数据源，页面需保留口径和风险提示；正式商用前应确认授权或切换授权源。
- 事件分类基于标题关键词规则，可能误分类；后续可升级为规则+LLM/模型复核，但首期不引入 LLM 成本。
- 非交易日事件映射到下一交易日用于收益计算，需在 UI 中显式提示，避免用户误读为事件当天交易。
