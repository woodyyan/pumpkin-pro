...
**背景**: 旧主站导航把「行情看板、选股器、回测引擎、策略库、持仓管理」平铺在顶栏，桌面与移动端还各自维护一套数组。随着 AI 页面、自选股、四象限独立入口的加入，旧结构已难以表达产品层级，也让 `_app.js` 承担了过多导航状态与渲染职责。

**决策**:
1. 主站一级导航固定为「卧龙AI / 看板 / 跟踪 / 选股 / 更多」。
2. 二级导航统一由 `frontend/lib/navigation.js` 单一配置源驱动，桌面端和移动端共享同一套路由、badge 与 active 规则。
3. 桌面端采用 hover 展开下拉，点击一级导航只负责展开/收起，不跳转默认页。
4. 移动端一期保留汉堡入口，菜单内部改为分组折叠，一次只展开一个分组；当前页面所属分组在打开菜单时默认展开。
5. `市场行情` 一期继续复用 `/live-trading`，不引入 `/market`；`四象限` 与 `自选股` 先落独立占位页，二期再迁移真实功能。
6. 新增 AI 相关页面采用语义化路径：`/ai/analysis`、`/ai/picker`、`/ai/backtest`；占位页统一只展示标题与「敬请期待」。

**原因**:
1. 导航配置集中后，PC / Mobile 不会再出现顺序漂移、badge 规则不一致或 active 归属分叉。
2. 五组信息架构比功能平铺更符合产品认知：AI 能力、看板、跟踪、选股工具和低频入口各自分层清晰。
3. 保留 `/live-trading` 可以避免搜索、个股详情、历史外链与现有测试在一期内产生大范围回归。
4. 占位页先落路由与导航入口，能在不牵动后端和重业务页面的前提下完成信息架构重构。

**替代方案**:
- 继续在 `_app.js` 里维护桌面数组 + more 菜单 + 移动菜单：否决，扩展 AI / 自选股 / 四象限后会继续膨胀且难测试。
- 桌面端点击一级导航直接跳默认页：否决，用户已明确要求 hover 自动展开，点击只负责开合更符合分组导航模型。
- 移动端一期直接上底部 Tab + 二级面板：暂不采用，当前网页端改动过大，先用汉堡 + 分组折叠控制风险。
- 一期同时把 `市场行情` 改到 `/market`：否决，语义更好但回归面过大，应放到后续路由升级阶段。

**影响**:
- `_app.js` 需要拆出导航配置和菜单组件，保留布局、账号、搜索、主题切换等壳层职责。
- 新增 `DesktopNavMenu.js`、`MobileNavMenu.js`、`NavDropdown.js`、`ComingSoonPage.js` 与对应占位页路由。
- 导航测试应从读取 `_app.js` 硬编码数组，改为校验统一导航配置与 active/badge 规则。
