...
## BP-023: 历史重算从共享缓存取最新价格，导致目标交易日价格错配

**模式**: 历史任务已将 `source_trade_date` 传给外部日线请求，但最终产出仍从可被当前任务复用的长期缓存读取“最新有效价格”。缓存含有目标日之后的数据时，结果任务本身会成功，却携带晚于目标日的 `price_trade_date`；下游严格校验 `price_trade_date == source_trade_date` 后持续显示缺价。

**案例**: Sim Portfolio v2 市场日历选择 A 股历史信号日重拉四象限后，Quant 成功回调，但日历仍显示“缺收盘价”。原因是 `_latest_cached_close()` 未限制目标日期，取到了共享缓存中的后续交易日收盘价。

**教训**:
- 历史重算的日期必须贯彻到“最终产出取价”环节，不能只约束外部请求的 `end_date`。
- 对需要严格日级口径的产出，目标日无精确价格时必须显式缺失，禁止回退前值或使用未来值。
- 历史重算应使用 `market + source_trade_date` 隔离缓存，避免污染当前计算状态，也避免复用未来日线。
- 回归测试应预置“目标日前后均有缓存数据”的场景，断言最终 `price_trade_date` 精确等于目标日期。

**适用场景**: 所有使用长期行情缓存的历史榜单、四象限、策略重算、模拟组合和回测任务。


## BP-024: 上游快照修复后未失效下游 blocked 批次

**模式**: Pipeline 首次读取上游数据缺失时，会持久化 `blocked` 批次；上游快照随后被异步回填，但下游查询优先读取已有批次，仅在批次不存在时才从上游重新构建，导致 UI 持续展示过期阻断状态。

**案例**: Sim Portfolio v2 市场日历中，历史日期四象限重建成功且 `quadrant_ranking_snapshots` 已有 50 条精确收盘价记录，但旧 `sim_portfolio_v2_signal_batches` 仍为 `blocked/candidate_count=0`，日历继续提示缺少信号快照。

**教训**:
- 上游异步回填成功后，必须显式失效或重建依赖该日期的数据批次；不能只依赖下游的“仅缺失时构建”逻辑。
- 失效范围必须使用业务键 `market + source_trade_date`，不能按 `computed_at` 或全局最新批次推断。
...
## BP-025: 同一业务口径的名称过滤在两条链路漂移

**模式**: 「退市股如何识别」这类名称规则在系统里有两处独立实现：四象限 `universe_filter.is_st_name` 用 `"退" in name`（覆盖「退市」前缀 + 「X退」后缀），而 Factor Lab `is_delisted_name` 只用 `startswith("退市")`。四象限筛掉的退市整理股（天龙退/恒久退/赛隆退等 92 只）通过 Factor Lab 选股进入单因子指数成分股（小市值因子最易吸进），退市后日线断更，指数整月「部分数据」。

**教训**:
- 业务口径类判定（ST/退市/停牌/板块归属）必须全仓唯一实现或显式对齐注释；新增一条链路的过滤时，先查其他链路同名判定。
- 「X退」后缀是现行交易所退市整理期命名惯例，只匹配「退市」前缀必然漏判。
- 小市值/低价类因子是退市股、问题股的天然聚集地，选股池过滤缺陷会在这类因子上最先、最严重地暴露。

**适用场景**: 所有跨链路（四象限 / Factor Lab / 选股器 / 指数构建）共享股票池口径的过滤规则。

## BP-026: 部署层 env 把多源降级链钉死单源

**模式**: 代码里设计了 `[baostock → tencent → eastmoney → akshare]` 降级链，但 docker-compose 用 `FACTOR_LAB_DAILY_BARS_SOURCE:-tencent` 把 Factor Lab 日线钉死腾讯单源。腾讯 fqkline 接口大面积 501 时无降级，全市场日线连续两个交易日只有 ~75% 覆盖，并连带 phase1 选股快照 1,126 只 `no_snapshot_date_bar` 失败。baostock 配额正常（10% 用量、无黑名单）却从未被调用。

**教训**:
- 降级链的价值在于「单点故障时自动绕行」；任何部署层（compose/k8s/env）把链钉成单源都等于废掉降级设计。
- 部署层覆盖代码默认值时，必须在 PR/变更记录里显式评估「这个覆盖是否削弱了容错路径」。
- 排障时看到「所有数据都来自同一个 source」，应优先检查部署 env 而不是假设 provider 选择逻辑失效。

**适用场景**: 所有「代码默认多源/多路径 + 部署 env 可覆盖」的数据源、降级、重试配置。

## BP-027: SQLite btree 损坏表现为「查询成功但结果残缺」，热备份静默失败

**模式**: 生产 `data/pumpkin.db` 的 `factor_daily_bars` 表及其主键索引（btree tree 79/80）损坏。最危险的形态是：**按索引范围扫描的查询正常返回，但结果集残缺**（同日日线 4,355 只被读成 3,280 只），系统看似在工作；而全表扫描（`COUNT(*)`、`VACUUM INTO` 热备份）直接报 `database disk image is malformed (11)`。损坏还导致 `.backup` 副本同样损坏；只有 `.recover` 能把行救到 `lost_and_found`（表根页损坏时整表行都会进 lost_and_found，需按 nfield 过滤重建，且会有少量不可恢复行与陈旧页幻影行，需与干净快照交叉验证）。

**案例**: 2026-07-25 因子指数修复验证时发现。`.recover` 重建后与干净快照对比：07-14~20 共有行 21,724 条中 108 条 close 不一致（0.5% 幻影行），约 24 只/日不可恢复。最终采用「恢复快照 + 管道自愈重抓」路径而非保留 recover 结果。

**教训**:
- 「查询能跑」不等于「数据完整」；对核心表应定期 `PRAGMA quick_check`（或比对同日 distinct 覆盖数的历史水位），把损坏暴露从「备份失败时」提前到「发生时」。
- 热备份任务必须校验 `VACUUM INTO` 成功并在失败时告警——否则损坏期间备份链静默中断，发现时已无新鲜干净备份。
- `.recover` 不是无损的：会混入陈旧页幻影行（INSERT OR REPLACE 的旧版本），恢复结果必须与干净快照交叉验证，不能直接用。
- 恢复优先级：干净快照 + 管道自愈重抓 > `.recover` 重建。管道能重抓的数据（日线/快照）不需要冒险用幻影行。

**适用场景**: 所有以 SQLite 为生产存储、依赖 VACUUM INTO/.backup 做热备份的链路；以及任何「范围查询结果数」可作为健康指标的离线数据表。

## BP-028: 物化行「先到先得、写过不重算」，链式物化需级联重算

**模式**: 日级物化任务以「目标日期是否已出现在源表（任何一行）」作为可计算信号，且对已存在的物化行永久跳过。上游数据晚到/残缺时落库的 partial 行永不自愈；对 NAV 这类链式物化（当日值依赖前一日值），单日修正后其后所有行都已污染，仅覆写单行无法修复。

**案例**: 单因子指数 `computeDailyForTradeDate`：`ListTradeDates` 取 `factor_daily_bars` 全表 distinct 日期（任一只股票有日线即算该交易日存在），NAV worker 在日线未补齐时抢先落库（0/50 有效也落库），`existing != nil → return` 永久跳过。页面展示的「X/50 缺日线」是该行首次计算瞬间的快照，与当前真实覆盖无关。

**修复**（决策 33）:
1. 计算前加源数据覆盖率门槛：当日覆盖显著低于基线时跳过该日、等补齐（宁可晚更新不可错更新）。
2. 物化行按状态区别对待：completed 才跳过；partial/pending 级联删除当日及之后全部行，由按日期升序的循环顺序重算，链从最近 completed 行续接。
3. 与 BP-024 的关系：BP-024 是「上游修复后未失效下游」，本条是「物化写时机过早 + 链式污染」，两者都要求物化层具备显式失效/重算能力，而不是「写过即真值」。

**教训**:
- 「可计算」的判定必须基于源数据对本批次的完整度，不能基于源表全局状态（如全表 distinct 日期）。
- 任何物化行都要回答「当源数据后来变好了，我如何被修正」；答不上来的物化链路必然积累脏行。
