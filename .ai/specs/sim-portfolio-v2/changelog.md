...
## 2026-07-15（缺陷修复：上游修复后过期信号批次未失效）

- **问题**: 某日期最初缺少排行榜快照时，日历会创建 `blocked` 信号批次；随后 Admin 重建该日期四象限虽成功并写入精确快照，日历仍优先读取旧批次，持续显示“缺少模拟组合信号快照”。
- **修复**: 四象限 `BulkSave` 的异步快照保存完成后，按 `market + source_trade_date` 删除对应 v2 信号批次及条目。日历/日期详情下次读取会从已更新的排行榜快照自动重建信号批次，不会沿用过期 `blocked` 状态。
- **测试**: 新增回归覆盖“旧 blocked 信号批次 → 上游快照补齐 → 信号批次失效 → 日历自动重建为 ok”。
- **运维**: 已经标红但上游快照已存在的日期，在发布后刷新日历/重新选择日期即可重建信号批次；无需再次重建四象限。
