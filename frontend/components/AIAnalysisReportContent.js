...
const SIGNAL_MAP = {
  ...
  hold: { label: '观望', arrow: '→', hint: '持仓不变', color: 'text-amber-600 dark:text-amber-300', bg: 'bg-amber-500/12', border: 'border-amber-400/40', dot: '🟡' },
}
...
export default function AIAnalysisReportContent({
  ...
  return (
    <section className={`rounded-2xl border ${sig.border} ${sig.bg} p-5 ${className}`}>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="mt-0.5 text-[11px] text-foreground-dim">{sig.hint}</div>
            <div className="mt-1.5 flex items-center gap-2">
              <span className="text-xs text-foreground-dim">置信度</span>
              <div className="h-2 w-32 overflow-hidden rounded-full bg-[var(--color-bg-hover)]">
                <div
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-foreground-dim">
        <span>数据时效：</span>
      ...
      {analysis.layer_scores && Object.keys(analysis.layer_scores).length > 0 && (
        <div className="mt-4 rounded-xl border border-border bg-[var(--color-bg-hover)] px-4 py-3.5">
              ...
              <div key={key} className="mt-2 first:mt-0">
                <div className="mb-1 flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[12px] font-medium text-foreground-muted">{meta.label}</span>
                    <span className="text-[10px] text-foreground-dim">({meta.weight})</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-semibold" style={{ color: dirColor }}>{dirLabel}</span>
                    <span className="text-[11px] font-mono text-foreground-dim">{ls.score > 0 ? '+' : ''}{ls.score}</span>
                    <span className="text-[10px] text-foreground-dim">置信度 {(ls.confidence * 100).toFixed(0)}%</span>
                  </div>
                </div>
                ...
                {ls.reason ? <p className="mt-1 text-[11px] leading-relaxed text-foreground-dim">{ls.reason}</p> : null}
              </div>
            )
          })}
          {analysis.total_score != null && (
            <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
              <span className="text-[11px] text-foreground-dim">加权综合评分</span>
              <span className={`text-sm font-bold font-mono ${analysis.total_score >= 0.5 ? 'text-negative' : analysis.total_score <= -0.5 ? 'text-emerald-400' : 'text-amber-400'}`}>
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-border bg-[var(--color-bg-hover)]">
        {allowLogicToggle ? (
          <button
            type="button"
          ...
          >
            <span className="text-xs font-medium text-foreground-muted">▾ 分析逻辑{!logicExpanded && '（点击展开）'}</span>
            <span className="text-[11px] text-foreground-dim">{logicExpanded ? '收起' : '展开'}</span>
          </button>
        ) : (
          <div className="px-4 py-2.5 text-xs font-medium text-foreground-muted">▾ 分析逻辑</div>
        )}
        {logicExpanded ? (
          <div className="px-4 pb-4">
            {logicLines.map((line, index) => (
              <p key={index} className="mt-2 text-[13px] leading-relaxed text-foreground-muted first:mt-0">• {line.replace(/^•\s*/, '')}</p>
            ))}
          </div>
        ) : null}
      </div>
      ...
      {Array.isArray(analysis.risk_warnings) && analysis.risk_warnings.length > 0 && (
        <div className="mt-4 rounded-xl border border-rose-400/25 bg-rose-500/8 px-4 py-3">
          <div className="mb-2 text-xs font-semibold text-negative/90">⚠️ 风险提示</div>
          {analysis.risk_warnings.map((warning, index) => (
            <p key={index} className="mt-1.5 text-[12px] leading-relaxed text-negative/70 first:mt-0">⚠️ {warning}</p>
          ))}
        </div>
      )}
      ...
      {ts.action_suggestion && (
        <div className="mt-4 rounded-xl border border-sky-400/20 bg-sky-500/5 px-4 py-3">
          ...
          {ts.time_horizon ? <div className="mt-2 text-[11px] text-foreground-dim">投资周期：{ts.time_horizon}</div> : null}
        </div>
      )}
        ...
        <div className="mt-4 rounded-xl border border-amber-400/20 bg-amber-500/5 px-4 py-3">
          ...
          {analysis.action_trigger.buy_trigger ? (
            <div className="mt-1.5 flex items-start gap-2 first:mt-0">
              ...
              <div>
                <span className="text-[11px] font-medium text-foreground-dim">买入触发</span>
                <p className="mt-0.5 text-[13px] leading-relaxed text-negative/80">{analysis.action_trigger.buy_trigger}</p>
              </div>
            </div>
          ) : null}
          {analysis.action_trigger.sell_trigger ? (
            <div className="mt-2.5 flex items-start gap-2 first:mt-0">
              <span className="mt-0.5 text-xs">🔴</span>
              <div>
                <span className="text-[11px] font-medium text-foreground-dim">卖出触发</span>
                <p className="mt-0.5 text-[13px] leading-relaxed text-positive/80">{analysis.action_trigger.sell_trigger}</p>
              </div>
            </div>
          ) : null}
        </div>
      ...
      {!hidePositionHint && completenessLabels.portfolio === 'has_position' ? (
        <div className="mt-3 rounded-lg border border-emerald-400/15 bg-emerald-500/5 px-3.5 py-2.5 text-[12px] text-positive/80">
          💡 你当前持有该股票，AI 已结合持仓盈亏给出针对性建议。
        </div>
      ) : null}

      <div className="mt-4 rounded-lg bg-[var(--color-bg-hover)] px-3.5 py-2.5 text-center text-[11px] text-foreground-dim">
        ⚠️ AI 分析仅供参考，不构成任何投资建议。市场有风险，投资需谨慎。
      </div>

      {showAnalysisTime && analysis.data_timestamp ? (
        <div className="mt-2 text-center text-[10px] text-foreground-disabled">分析时间：{formatDateTime(analysis.data_timestamp)}</div>
      ) : null}
    </section>
  )
}
