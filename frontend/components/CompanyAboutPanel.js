...
function AboutField({ label, value, children }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-[var(--color-bg-hover)] px-3 py-3">
      ...
      <div className="mt-1 break-words text-sm font-medium text-foreground/82">{content}</div>
    </div>
  )
}
...
function statusClass(status) {
  ...
  if (tone === 'listed') return 'border-emerald-300/25 bg-positive/10 text-positive'
...
export default function CompanyAboutPanel({ payload, loading = false, error = '', onClose }) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-5" data-company-about-panel="true">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-base font-semibold text-foreground">关于这家公司</h3>
      ...
      {loading ? (
        <div className="mt-4 rounded-xl border border-dashed border-border px-4 py-6 text-sm text-foreground-dim">公司资料加载中...</div>
      ) : error ? (
        <div className="mt-4 rounded-xl border border-amber-400/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">{error}</div>
      ) : !payload?.has_profile ? (
        <div className="mt-4 rounded-xl border border-dashed border-border px-4 py-6 text-sm text-foreground-dim">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
          <div className="rounded-xl border border-border bg-[var(--color-bg-hover)] p-4">
            <div className="text-[11px] text-foreground/38">一句话业务介绍</div>
            ...
            {profile?.business_scope ? (
              <details className="mt-3 group">
                <summary className="cursor-pointer text-xs text-foreground-dim transition hover:text-foreground-muted">查看原始业务资料</summary>
                <p className="mt-2 text-xs leading-6 text-foreground-dim">{profile.business_scope}</p>
              </details>
            ) : null}
          </div>
