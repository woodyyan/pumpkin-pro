...
export function InfoTip({
  ...
  return (
    <span className={`group/info-tip relative inline-flex align-middle ${className}`}>
      <button
        type="button"
        className={`inline-flex h-4 w-4 items-center justify-center rounded-full border border-border bg-[var(--color-bg-hover)] dark:border-[var(--color-border-strong)] text-[10px] font-semibold text-foreground-dim transition hover:border-primary/40 hover:text-primary focus:outline-none focus:ring-2 focus:ring-primary/25 ${iconClassName}`}
      ...
      <span className={`pointer-events-none absolute z-30 hidden rounded-lg border border-border bg-white dark:bg-[#101418]/95 px-3 py-2 text-[11px] font-normal leading-5 text-foreground-muted dark:text-foreground-muted shadow-2xl backdrop-blur-sm group-hover/info-tip:block group-focus-within/info-tip:block ${widthClassName} ${classes.panel} ${panelClassName}`}>
        {text}
