...
const PAGE_THEME = {
  shell: 'min-h-screen bg-[radial-gradient(circle_at_top,#19324a,transparent_42%),linear-gradient(180deg,#09131d,#0d1823_55%,#13283a)] px-4 py-10 text-white',
  card: 'mx-auto w-full max-w-[30rem] rounded-[2rem] border border-white/10 bg-[#0f1722]/90 p-6 shadow-[0_30px_80px_rgba(0,0,0,0.32)] backdrop-blur',
  input: 'w-full rounded-2xl border border-white/12 bg-white/5 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/35 focus:border-primary focus:bg-white/[0.07]',
      ...
      <main className={PAGE_THEME.shell}>
        <div className={PAGE_THEME.card}>
          <div className="mb-6 space-y-3">
            ...
            <p className="text-sm leading-7 text-white/70">输入你的注册邮箱后，我们会发送一封密码重置邮件。邮件里只包含重置链接和有效期说明，不会展示你的账号敏感信息。</p>
          </div>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            {error ? <div className="rounded-2xl border border-rose-400/20 bg-rose-400/10 px-4 py-3 text-sm text-rose-100">{error}{retryAfter > 0 ? `，请约 ${retryAfter} 秒后再试。` : ''}</div> : null}
          ...
          <div className="mt-6 flex items-center justify-between text-xs text-white/45">
            <a href="/" className="transition hover:text-white">返回首页</a>
            <a href="javascript:history.back()" className="transition hover:text-white">返回登录</a>
          </div>
        </div>
      </main>
    </>
